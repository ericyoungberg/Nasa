...
Nasa.Engine = (function(__export__) {
  ...
  __export__.check = function(route) {
    ...
    if(Nasa.Engine.location() === route) _execute(route);
    ...
    for(var i = 0; i < route_boom.length; i++) {
      ...
      if((index = route_boom[i].indexOf('*')) !== -1) {
        ...
        if(index === 0) {                                 // Wildcard at beginning
          ...
          if(location_boom[i].indexOf(end) !== location_boom[i].length - 1 - (end.length - 1)) return;
          ...
          // then this isn't the route you are looking for.
          if(location_boom[i].substr(0, beginning.length) !== beginning) return;
      ...
      if(route_boom[i] !== location_boom[i]) return;
    }
